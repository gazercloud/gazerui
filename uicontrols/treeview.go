...
func NewTreeView(parent uiinterfaces.Widget) *TreeView {
	...
	c.cellPadding = 0
	c.panelPadding = 0
	c.cache = ui.NewImageCache("TreeView")
	c.itemHeight = 20
	c.contentPadding = 3
...
func (c *TreeViewContent) drawNode(ctx ui.DrawContext, node *TreeNode, x int, y int) int {
	yOffset := 0
...
func (c *TreeViewContent) drawGrid(ctx ui.DrawContext) {
	xOffset := 0
...
func (c *TreeViewContent) drawPlus(ctx ui.DrawContext, x int, y int) {
	ctx.DrawImage(x, y, 5, 5, c.treeView.imgPlus)
}

func (c *TreeViewContent) drawMinus(ctx ui.DrawContext, x int, y int) {
	ctx.DrawImage(x, y, 5, 5, c.treeView.imgMinus)
}

func (c *TreeViewContent) drawIcon(ctx ui.DrawContext, x int, y int, img image.Image) {
	if img != nil {
...
func (c *TreeViewContent) MouseDown(event *uievents.MouseDownEvent) {
	...
	if dNode != nil {
		...
		c.lastMouseDownPoint = ui.Point{event.X, event.Y}
...
func (c *TreeViewHeader) MouseMove(event *uievents.MouseMoveEvent) {
	...
	if event.Y < c.treeView.itemHeight {
		if c.treeView.findColumnRightBorder(event.X, event.Y) >= 0 {
			c.Window().SetMouseCursor(ui.MouseCursorResizeHor)
		} else {
