...
type Table struct {
	...
	columns                []*TableColumn
	rows                   []*TableRow
	focusedCell            uiinterfaces.Widget
...
func (c *Table) btnClick(event *uievents.Event) {
...
func (c *Table) AddRow(name string) {
	...
	row.Cells = make([]uiinterfaces.Widget, 0)
...
func drawWidgetOnCanvasTest(ctx *canvas.CanvasDirect, w uiinterfaces.Widget) {
...
func (c *Table) widgetByPoint(x, y int) uiinterfaces.Widget {
...
func (c *Table) cellByPoint(x, y int) (int, int) {
	for rowIndex, row := range c.rows {
		for colIndex, _ := range c.columns {
