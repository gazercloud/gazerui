...
func NewTextBox(parent uiinterfaces.Widget) *TextBox {
	...
	menu := NewPopupMenu(&b)
	menu.AddItem("Cut", func(event *uievents.Event) {
		b.cutSelected()
	}, nil, "")
	menu.AddItem("Copy", func(event *uievents.Event) {
		b.copySelected()
	}, nil, "")
	menu.AddItem("Paste", func(event *uievents.Event) {
		b.paste()
	}, nil, "")
	menu.AddItem("Delete", func(event *uievents.Event) {
		if !b.readonly {
			b.modifyText(textboxModifyCommandDelete, uievents.KeyModifiers{}, nil)
		}
	}, nil, "")
	menu.AddItem("Select All", func(event *uievents.Event) {
		b.SelectAllText()
	}, nil, "")
...
func (c *TextBox) SetText(text string) {
	c.redraw()
	var modifiers uievents.KeyModifiers
...
func (c *TextBox) cutSelected() {
	...
	if !c.readonly {
		c.modifyText(textboxModifyCommandDelete, uievents.KeyModifiers{}, nil)
	}
}
...
func (c *TextBox) paste() {
	...
	c.modifyText(textboxModifyCommandInsertString, uievents.KeyModifiers{
		Shift:   false,
		Control: true,
		Alt:     false,
	}, str)
...
func (c *TextBox) MouseDown(event *uievents.MouseDownEvent) {
	if event.Button == uievents.MouseButtonLeft {
		c.redraw()
...
func (c *TextBox) moveCursorNearPoint(x, y int, modifiers uievents.KeyModifiers) {
...
func (c *TextBox) insertReturn(modifiers uievents.KeyModifiers) bool {
	if !c.multiline {
		return false
	}
...
func (c *TextBox) moveCursor(posX int, posY int, modifiers uievents.KeyModifiers) {
...
func (c *TextBox) removeSelectedText(modifiers uievents.KeyModifiers) (bool, []string, int, int) {
	lines := make([]string, 0)
	modified := false
	selection := c.selectionRange()
...
func (c *TextBox) modifyText(cmd textboxModifyCommand, modifiers uievents.KeyModifiers, data interface{}) {
	c.redraw()
