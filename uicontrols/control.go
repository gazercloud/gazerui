...
type Control struct {
	...
	parent    uiinterfaces.Widget
	OwnWindow uiinterfaces.Window

	theme  string
	widget uiinterfaces.Widget
	...
	onMouseWheel    func(event *uievents.MouseWheelEvent)
	onMouseMove     func(event *uievents.MouseMoveEvent)
	onMouseDown     func(event *uievents.MouseDownEvent)
	onMouseUp       func(event *uievents.MouseUpEvent)
	onMouseClick    func(event *uievents.MouseClickEvent)
	onMouseDblClick func(event *uievents.MouseDblClickEvent)
	onKeyChar       func(event *uievents.KeyCharEvent)
	onKeyDown       func(event *uievents.KeyDownEvent) bool
	onKeyUp         func(event *uievents.KeyUpEvent)
	onMouseEnter    func()
	onMouseLeave    func()

	onPreMouseWheel    func(event *uievents.MouseWheelEvent)
	onPreMouseMove     func(event *uievents.MouseMoveEvent)
	onPreMouseDown     func(event *uievents.MouseDownEvent)
	onPreMouseUp       func(event *uievents.MouseUpEvent)
	onPreMouseClick    func(event *uievents.MouseClickEvent)
	onPreMouseDblClick func(event *uievents.MouseDblClickEvent)
	onPreKeyChar       func(event *uievents.KeyCharEvent)
	onPreKeyDown       func(event *uievents.KeyDownEvent)
	onPreKeyUp         func(event *uievents.KeyUpEvent)
...
func (c *Control) InitControl(parent uiinterfaces.Widget, w uiinterfaces.Widget) {
	...
	if parent == nil {
		fmt.Errorf("no parent for control")
	}
...
func (c *Control) UpdateStyle() {
	...
	if c.contextMenu != nil {
		if popupMenu, ok := c.contextMenu.(*PopupMenu); ok {
			uistyles.InitDefaultStyle(popupMenu)
...
func (c *Control) SetHover(hover bool) {
	if hover {
		if c.cursor != ui.MouseCursorNotDefined && hover {
			c.Window().SetMouseCursor(c.cursor)
		}
	} else {
		if c.cursor != ui.MouseCursorNotDefined {
...
func (c *Control) ProcessMouseClick(event *uievents.MouseClickEvent) {
	...
	if !me.Ignore {
		...
		if event.Button == uievents.MouseButtonRight {
...
func (c *Control) DrawScrollBars(ctx ui.DrawContext) {
	...
	if c.innerSizeOverloaded {
		...
		scrollBarsColor := uievents.ColorWithAlpha(c.leftBorderColor.Color(), 192)
...
func (c *Control) translateMouseEvent(event *uievents.MouseEvent) {
...
/*func (c *Control) DrawGL(ctx *ui.DrawContextGL) {
	ctx.SetColor(colornames.Blue)
	ctx.FillRect(0, 0, c.Width(), c.Height())

	//ui.UpdateResolution(c.Width(), c.Height())
	//ui.DrawTextInRect(c.Window().Window(), 0, 0, c.Width(), c.Height(), c.ControlType(), 5)
	//ui.DrawText(c.Window().Window(), c.Width() / 2, c.Height() / 2, c.ControlType(), 5)
}
*/
